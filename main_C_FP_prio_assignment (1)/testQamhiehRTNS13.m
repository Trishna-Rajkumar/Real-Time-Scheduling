...
function schedQam = testQamhiehRTNS13()
% performs the test by Qamhieh et al. (RTNS 2013)
    ...
    for i = 1 : length(task)
        ...
        DBF(i) = getCumulativeDBF();
        carryIn(i) = getCumulativeCarryIn(i);
        
        if DBF(i) + carryIn(i) > task(i).D
            schedQam = 0;
            return;
        end
    end
